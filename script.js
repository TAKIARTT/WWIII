...
function activateBoostByAd(button) {
  ...
  AdController.show()
    .then(result => {
      msg.textContent = " Ad finished! Claim your reward:";
      btn.textContent = " Claim Boost 2";
    ...
    .catch(() => {
      msg.textContent = " Failed to load ad.";
      setTimeout(() => {
        modal.style.display = "none";
        showToast("Ad failed");
        adPurpose = null;
  ...
  AdController.show()
    .then((result) => {
      if (result.done) {
        ...
        messageEl.textContent = " Ad skipped. No reward given.";
        closeBtn.style.display = "none";
