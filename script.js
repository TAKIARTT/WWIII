...
function activateBoost(source, button) {
  ...
  if (rocket) {
    ...
    rocket.classList.add("rocket-boosted"); //  ÃÖáæÇ ÊÃËêÑ ÇäÔÙäÉ Ùäé ÇäÕÇÑèÎ
    setTimeout(() => rocket.classList.remove("fly"), 1000);
  }

  if (button) {
    button.classList.add("active-boost"); //  ÔÙäÉ ÇäÒÑ
  }
...
function showDailyPopup(streak, reward) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    // ÅÐÇ ÇäåãÇáÃÉ äçÐÇ Çäêèå Êå ÇÓÊäÇåçÇ ÓÇÈâëÇ (êèå Çäêèå áâ×)
...
lottie.loadAnimation({
  ...
  loop: true,
  autoplay: true,
  path: 'gift.json' // åÓÇÑ åäá JSON ÇäÎÇÕ Èã
});
...
//  ÌèÇÆÒ ÇäÙÌäÉ èÊèÒêÙçÇ ÍÓÈ ÇäèÒæ (ÇÍÊåÇäÇÊã)
const prizes = [
...
function addActivity(type, value) {
  ...
  //  ÏåÌ Tap reward áê ÇäÓ×Ñ ÇäÃèä áâ×
  if (
    type === "tap" &&
...
function openAdModal(purpose = null, button = null) {
  if (adSpinsUsed >= maxAdSpins && purpose === "spin") {
    showToast(" No more ads today. Come back tomorrow!");
    return;
  }

  adPurpose = purpose; //  æÍáØ çä ÇäÖÚ× åæ ÃÌä spin Ãè boost
  lastBoostButton = button; // æÍÊáØ ÈÒÑ boost ÇäåÍÏÏ
  ...
  let countdown = 10;
  countdownEl.style.display = "none"; //  êÎáê ÇäæÕ ÇäÑâåê
...
document.getElementById("watch-ad-btn").addEventListener("click", showAd);
  ...
  AdController.show()
    .then((result) => {
      if (result.done) {
        ...
        messageEl.textContent = "⚠️ Ad skipped or closed. Exit?";
