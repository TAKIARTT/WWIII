...
function closeOnOutsideClick(event, wrapper) {
  const content = wrapper.querySelector(".modal-content");
  const closeBtn = wrapper.querySelector(".close-btn");

  // ≈–«  Â «‰÷⁄◊ Œ«—Ã «‰ÂÕ ËÈ¨ √⁄‰‚ «‰Ê«·–…
  if (!content.contains(event.target)) {
    closeBtn?.click(); // ÊÕ·Ò“ «‰“— Ê·”Á
  }
}
  ...
  AdController.show()
    .then((result) => {
      if (result.done) {
        ...
        // ÂÀ«‰: √Ÿ◊Á Ê‚«◊ √Ë ·ŸÒ‰ Boost
        showToast(" Ad watched! Reward granted.");
        addActivity("ad", "+1 spin");
        spinsToday--; // ≈–« „«Ê  “Í«œ… Ÿœœ «‰‰·Ò«  ÂÀ‰«Î
        updateSpinsUI();
      }
    })
    .catch((error) => {
      console.warn("Ad error:", error);
      showToast(" Ad failed or skipped.");
    });
}
